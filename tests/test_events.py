...
class TestListEvents:
    ...
    def test_list_events_no_auth(self, client, app, user_a, mock_ext):
        gid = make_game(app)
...
class TestGetEvent:
    ...
    def test_get_event_ok(self, client, event_id):
        resp = client.get(f'/api/v1/events/{event_id}')
        assert resp.status_code == 200
        assert resp.get_json()['data']['id'] == event_id

    def test_get_event_unknown_404(self, client):
...
class TestCloseReopenEvent:

    def test_close_event_ok(self, client, event_id, user_a):
        resp = client.post(f'/api/v1/events/{event_id}/close',
                           headers=auth(user_a['token']))
        assert resp.status_code == 200
        assert resp.get_json()['data']['status'] == 'completed'

    def test_close_not_creator_403(self, client, event_id, user_b):
        resp = client.post(f'/api/v1/events/{event_id}/close',
                           headers=auth(user_b['token']))
        assert resp.status_code == 403

    def test_reopen_event_ok(self, client, event_id, user_a):
        client.post(f'/api/v1/events/{event_id}/close', headers=auth(user_a['token']))
        resp = client.post(f'/api/v1/events/{event_id}/open',
                           headers=auth(user_a['token']))
        assert resp.status_code == 200
